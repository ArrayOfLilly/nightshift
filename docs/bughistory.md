...
**Státusz:** ✅ KÉSZ — BH session: `AboutView.swift` új fájl (`AboutWindowID` enum, `AboutCommands`
struct `CommandGroup(replacing: .appInfo)`); app ikon, név, verzió/build, tagline, Developer mailto
link, Images → Freepik attribution link, © 2026 footer. `MARKETING_VERSION = 0.9.2`,
`CURRENT_PROJECT_VERSION = 2` (Debug + Release), `INFOPLIST_KEY_CFBundleDisplayName = NightShift`.
Git commit: `7ab7b65`
...
**Státusz:** ✅ KÉSZ — BN session: "Save and quit" ág elé `shouldSaveOnDisappear = false` bekerült,
a `commitSave()` elé. `.onDisappear` double-call megszűnt, az utolsó módosítás megbízhatóan mentésre
kerül. Git commit: `c8b3d5e`
...
**Státusz:** ✅ KÉSZ — BN session: `SnippetEditSheet.snippet` `let` → `@State private var`;
`commitSave()` sikeres mentés után `self.snippet = s`, így a következő `commitSave()` már update-el,
nem újat generál. `SnippetsView.showNewSheet` `onSave` closure: `append` → id-alapú upsert (mert a
sheet-fix után is duplikálna, ha ugyanazt az id-t kétszer appendelné). Git commit: `c8b3d5e`
